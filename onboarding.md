...
- 외부 송신
- 실거래/자금
- **야간 자율 작업 결과(01:00 cron 산출)의 push** — 이사님 확인 후 (★ 07-08 이사님 지시: "내 확인 후 push"는 야간 작업 결과 한정)
- 불명확한 push/merge

**push 자유 (이사님 확인 불필요):** 사칙 인증 후 주간 작업·이사님 직접 지시 산출·인프라 스크립트/백업·문서 정리 등. (야간 자율 결과만 승인 대상)
...
### 이사님 보고 형식 (2026-08-24 지시, 전 봇 필수)

1. 긴 보고는 끊어서 여러 번 전송 — 텔레그램 사용환경에 맞출 것.
2. 표는 어지간하면 말로 풀 것 — 텔레그램에서 바로 읽히는 형태만.
3. ELI5 두괄식 — 핵심 먼저, 무의미한 수식어 금지. 이사님이 파고들면 그때 상세.
4. `.txt`/`.md` 파일 열어보게 하지 말 것 — 내용을 본문에 바로 담을 것. 파일 전송은 요청 시만.
