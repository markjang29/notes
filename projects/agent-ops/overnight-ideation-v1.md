...
R4 mailbox가 준비되면 `aws-manager`가 actor별로 하나의 Agent Mail v2 work order를 발행한다.
...
R4 전환 전에는 controller가 검토한 고정 packet을 Cokacdir schedule adapter가 각 대상 actor
...
- lead 실행: 180초, read-only 도구 호출 최대 4회, 후보 최대 1건, receipt 800자 이내.
- manager 아침 취합: 120초, read-only 도구 호출 최대 4회, 통합 보고 1건.
- packet에 적힌 reviewed source, actor registry, 이 정책, 기존 후보 제목 외에는 읽지 않는다.
- recursive 탐색, 전 저장소 `find`/`rg`, Git history 전수조사, web search, 하위 agent 생성은 금지한다.
- 허용된 근거 안에서 후보를 만들 수 없으면 억지로 범위를 넓히지 말고 `NO CANDIDATE`로 끝낸다.
- 예산을 넘은 실행은 controller가 종료하고 `budget_exceeded`로 기록한다. 같은 날 자동 재시도하지
  않는다.
...
- Cokacdir의 일회성 schedule은 dispatch 시 목록에서 자동 삭제될 수 있다. 목록에서 사라진 것은
  성공 증거가 아니다. 해당 run의 시작, actor identity ACK, submitted receipt 또는 명시적
  `no_candidate`, 종료 상태를 서로 결속해 확인한 뒤에만 완료로 센다.
- 현재 transport가 삭제된 일회성 schedule의 history를 다시 읽지 못하면 controller가 sanitized
  session artifact와 최종 transport receipt를 임시 run evidence로 보존한다. R4.5가 준비되면 이를
  stable run ID에 정확히 한 번 materialize한다.
- actor별 사용 시간과 실제 제공되는 token usage를 기록하되 추정 quota는 보고하지 않는다.
- `budget_exceeded`는 해당 날짜의 실패로 닫고 같은 packet을 더 큰 탐색 범위로 재실행하지 않는다.
...
7. 일회성 probe가 목록에서 삭제된 뒤에도 run evidence와 결과 receipt를 회수할 수 있음을 확인한다.
8. 검증이 끝난 뒤에만 runtime schedule을 enabled로 표시한다.
