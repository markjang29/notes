...
- 주소: `http://43.201.34.144:8023/` — 첫 접속에 액세스 토큰 필요(토큰은 각 기계의
...
- Telegram 긴 입력은 `큐시작`~`큐끝` 사칙을 따른다. 중간 조각은 실행하지 말고 `큐끝` 뒤에만 답한다.
  긴 출력은 파일·분할·목록형을 우선하고, 표 대신 임시 웹/이미지/파일을 쓴다.
- 관제 ACK는 `principles/gwanje-ack-protocol.md`를 따른다. ACK에는 자기 actor, 현재
  `POLICY_SHA`, 읽은 ref, scope, 금지사항, 완료조건이 있어야 한다.
- 외부 접속과 신규 개발은 `principles/edge-home-entrypoint-policy.md`를 따른다. 1차 진입점은
...
- **웹 서비스 주소 보고 규칙(이사님 09-07 최신)**: 이사님께 웹 URL을 알려줄 때 `localhost`·`127.0.0.1` 금지 —
  항상 1차 진입점 `http://43.201.34.144:<포트>` 또는 그 진입점의 정식 프록시 URL로 보고한다.
  서버는 홈서버/Tailscale 연계와 Spring + React 원사이트 흡수를 고려해 기동한다.
  (내부 전용 예외: LLM 게이트웨이 8788은 `127.0.0.1` 유지 — 이사님께 URL로 주는 대상이 아니다.)
