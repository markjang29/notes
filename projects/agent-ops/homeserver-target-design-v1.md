...
- **실물 확정(이사님 09-06)**: 신규 AWS 엣지 인스턴스 생성 완료 — **512MB·ubuntu 유저**,
  외부 입구 전용(서비스 상주 금지, tailnet으로 홈 리눅스에 실서빙 위임). 접속 테스트 대기
  (키파일+IP 수령 시). **기존 AWS 8GB(13.125.131.126)는 셧다운 예정** → 봇·관제의 홈 이관이
  §6 4단계가 된다.
