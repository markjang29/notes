...
2. 파뱃 PC에 유입되는 자산의 로컬 창고 관리 (`C:\Users\n100\assets\`)
3. 자산의 홈서버·클라우드 보관·동기화 구조 관리 — 백업정책 v2(09-06 확정):
   firewin 원본 → gmlnx 운영서버 미러 → Google Drive 콜드, 삭제 금지·압축 우선.
...
- **큐 수신 규칙** (이사님 09-07): 이사님이 **"큐시작"**을 보내면 그 뒤로 오는 메시지들(텔레그램 잘림으로
  여러 조각으로 나뉜 긴 전달)은 **응답하지 말고 전부 수집**하고, **"큐끝"**이 도착하면 그때 전체를
  처리해 한 번에 응답한다. 큐 도중에는 중간 반응·요약·확인 질문 금지.
