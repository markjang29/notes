...
4. 작업·보고·이관 산출물에 `POLICY_SHA`가 빠지지 않았는지 점검한다.
5. Telegram 큐 수신·응답 같은 대화 사칙을 누락 없이 정책 인덱스와 온보딩 문서에 연결한다.
6. 정책 변경이 필요한 경우, 변경안·영향·대상 봇·검수 방법을 ELI5로 요약해 이사님 또는 매니저에게
   전달한다.
7. 정책 전파는 회의방/매니저 경유를 우선하되, Telegram 알림만으로 완료를 주장하지 않는다.
