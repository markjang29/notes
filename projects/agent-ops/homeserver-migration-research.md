...
## 8. GPT 답변 평가·종합 (09-04 — 이사님 교차 검증용 수신)

**근거 검증(먼저)**: 인용된 Anthropic 실험은 실재한다 — orchestrator-worker 다중 에이전트 논문·
장기 작업 하네스 엔지니어링 문서 모두 확인. 단 **"Planner→Builder→Evaluator"라는 용어는 Anthropic
원문이 아니라 GPT의 재정리**다(본질은 동일: 역할 분리·구조화된 상태 전달·별도 검증 패스).

- **GPT의 진단 — 채택**: "문제의 중심은 AI 20개를 잘 부리는 법이 아니라, 20개가 무엇을 해도 구조가
  망가지지 않는 하네스" + "요구의 의미 변질"(사이트 합치기→UI만 합침, 데이터 통합→컬렉션 하나 추가,
  객체 통합→facade만 씌움) — 이사님이 실제 겪은 사례와 정확히 일치. Gemini보다 한 층 깊은 진단.
  각 AI가 개발자+아키텍트+DBA+DevOps+PM을 동시에 한다는 지적도 우리 조직에 그대로 해당(팀장이
  설계까지 잡는 순간이 있었음).
- **Gemini vs GPT 관계 — 충돌 아님, 층이 다름**: Gemini = 인프라·아키텍처 통합(어디에 무엇을 모을까),
  GPT = 실행 하네스(일을 어떤 형식으로 시킬까). 둘 다 필요하며 순서는 하네스가 먼저다 — 형식 없이
  모은 창고는 다시 흩어진다.
- **우리 조직 대입**: 이미 있는 것 — Planner=매니저, Builder=팀장·zcode·firebat, Evaluator=감사
  (모델도 분리: codex×claude 교차 검증), 규칙 파일·SSOT 원장 3종, 지라칸반.
  **없는 것 3가지** — ①Task Contract 표준(repo·인수조건·허용 모듈·원천·예상 테스트·금지 변경·산출
  형식의 7필드 티켓) ②자동 게이트(repo별 CI: 빌드·테스트·린트·브랜치 보호) ③작업 격리(병행 작업 시
  git worktree).
- **채택 제안**: 하네스 표준 ADR 신설 — 티켓 7필드 필수화(work-queue 템플릿 개정), CI 최소판 도입,
  관제 5축과 연결(담당업무=Task Contract 뷰, 의사결정=게이트 통과 증명 뷰). Evaluator는 기존 감사봇
  그대로 충족.
- **통찰**: 조직도(사람형 협업)에서 파이프라인(공장형 형식)으로 — 이사님이 시나리오 팀에 정의한
...
3. AWS 완전 퇴출인지, 아니면 "텔레그램 관문+LLM 키 게이트웨이만 AWS 잔존"인지 — 이것이 이관 설계의
   최대 분기점.
4. **GPT형 하네스 표준 채택** — 티켓 7필드(Task Contract) 필수화 + CI 최소판 + 병행 작업 git worktree
   격리를 조직 표준으로(관제 5축과 연결). 매니저 바로 착수 가능(티켓 템플릿 개정은 제 소관).
