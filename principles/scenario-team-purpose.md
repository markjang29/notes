...
## 5.1 상위 규칙 — 자산 사용 전 컨펌 (07-07 이사님 확정)

> *"그렇게 설정된 거를 가져와서 말할 거면 내 컨펌이 먼저 필요하고, 내가 공감한 걸 토대로 해야지."*

- **올려준 것 ≠ 공감한 것 ≠ 아는 것.** 이사님이 RISU 자산(602점)을 제공했지만, 각 자산에 공감/선택한 것은 아니다. 이사님은 602개를 외우지 않는다.
- **방법론 논의 단계**에서는 특정 자산(캐릭터 등)을 이사님이 아는 것처럼 언급 금지.
- 특정 자산을 예시/창작/디벨롭에 끌어오려면:
  1. **이사님 컨펌 먼저** (이 자산을 써도 되는지)
  2. **이사님이 공감/선택한 자산**을 토대로
- 자산 언급 시는 **반드시 배경 설명** 동반 ("OO — 000 자산, 000 서사").
- 매트릭스(제품용 자산 공장) 설계의 진열/추천 구조도 이 원칙 기반 — 팀장이 자산 임의 선정해 이사님 것처럼 창작하지 않는다.

## 5.2 자산 소유/사용 경계 — 시나리오팀은 독점자가 아니라 이력관리자 (07-10 이사님 정정)

> *"시나리오 자산을 시나리오 팀장만 써야되는건 아니야. 시나리오 팀장이 이력관리 되고, 시나리오 만들 때 가이드를 지키면 됨."*

- **시나리오 자산은 시나리오팀 전용 독점물이 아니다.** RPG·trader·매니저 등 다른 팀도, 이사님 컨펌과 가이드 준수 범위 안에서 시나리오 자산을 참조·응용할 수 있다.
- **시나리오팀의 책임은 독점 사용이 아니라 이력관리·출처관리·가이드 관리**다. 어떤 자산이 어디서 왔고, 어떤 제품/장면/참모 슬롯에 어떻게 쓰였는지 PROVENANCE와 사용 이력을 남긴다.
- 다른 팀이 시나리오 자산을 사용할 때도 다음은 지킨다:
  1. 사칙 5.1의 **이사님 컨펌/공감/선택 기준**
  2. 자산 배경 설명과 PROVENANCE 기록
  3. 시나리오팀 가이드(자산 실제 내용 기반, 메타만 사용 금지)
  4. 제품 이식·디벨롭은 컨펌 후 진행
- 따라서 "시나리오팀만 자산을 써야 한다"가 아니라, **시나리오팀이 자산 사용의 장부와 품질 가이드를 책임진다**가 정확한 기준이다.
