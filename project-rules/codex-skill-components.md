...
가용성 요청 시점까지 기다리지 않는다. root post 선정은 먼저 읽기 전용으로 수행하고, 원장을
바꾸기 직전에 AWS live control/config/policy와 controller lease를 다시 확인해 post/source/bucket에
묶인 60초짜리 one-use preselection permit을 발급한다. `start-post`는 이 permit과 현재 로컬
opaque lease의 SHA-256이 정확히 일치할 때만 gate를 닫고 permit을 소비한다. pause/disable/
emergency, stale digest, 다른 lease, 다른 bucket, 기존 blocker 중 하나라도 있으면 원장을
변경하지 않는다. 다운로드·브라우저·파서 단계에서 막혀도 다음 스케줄이 다른 게시글을 고를 수
없어야 한다.
승격 asset id와 AWS request id가 정해지면 `bind-asset`으로 같은 행을
`availability_testing`에 전환하고, terminal 판정에서만 gate를 다시 연다.

## Matrix factory와 균형 수집 스킬 구성요소 (2026-07-14)

정본은 scenario 저장소 commit `7744a0dfa3a31804780ad92b6b19369d82ad7717`이다. Codex와
Claude는 각각 사용자 설치본을 쓰되 `.agents/skills`가 유일한 정본이고 `.claude/skills`와
`agent-skills.lock.json`은 자동 생성 mirror다. ZCode용 mirror는 만들지 않고 repo의 `.agents`를
직접 읽게 한다.

- `$arca-collection`: live 로그인/QR, AWS sync/control, 단일 lease, 신·레거시 선정, 한 게시글
  deep dive, 원본 download/hash/parser, 사용 recipe와 Matrix capsule, AWS 가용성, terminal
  lifecycle, Git push를 한 단위로 닫는다.
- `$matrix-factory`: reviewed/active/usable 자산만 읽어 `world`, `character_archetype`, `persona`,
  `module`, `narrative_blueprint`을 조합한다. live 수집·로그인·승격 권한은 없다.
- 세계 융합은 `preserve/translate/override/bridge/exclude`, 캐릭터 적응은
  `native/translate_surface/fork`를 명시한다. exact version/ref/source SHA와 capabilities
  requires/provides/conflicts, state/memory policy를 manifest에 고정한다.
- compiler는 seed·input commit·예산·target mode·fusion 결정을 묶어 결정론적 prompt와
  `matrix-run-manifest-v1`을 만든다. 필수 투영이 UTF-8 byte ceiling을 넘으면 fail-closed하고,
  선택 항목만 안정 순서로 pruning한다. 실제 provider token/response는 run_id로 연결한 외부
  runtime ledger에 두며 deterministic manifest에 섞지 않는다.
- 기본 일일 terminal mix는 신규 21, 레거시 재검증 14다. 신규는
  `8/5/3/2/2/1`, 레거시는 `module 6/character 4/preset 4`; 명시적 적격 후보 고갈 때만
  재배분한다. 열린 한 건은 날짜가 바뀌어도 항상 먼저 재개한다.
- AWS lease는 원문 `lease_id`를 최초 응답과 로컬 상태에만 두고 서버에는 SHA-256만 저장한다.
  renew/release는 holder+device+token exact match다. 예약 Agent 부모가 child 전체 수명 동안
  TTL의 1/3 간격으로 heartbeat하고 outer `finally`에서 release한다. child의 acquire/release는
  환경 guard가 API 호출 전에 차단하며 nested crawler는 renew만 한다.
- portable wrapper v3가 `Sync/Run/Agent/LeaseAcquire/LeaseRenew/LeaseRelease/Preselect`를
  제공한다. repo 위치는 환경변수, 사용자 runtime 포인터, cwd/부모 순으로 찾고 cookies,
  credentials, lease, permit, worker receipt는 `$HOME/.scenario-automation` 밖으로 내보내지 않는다.
- ZCode는 한 번의 고정 context 비용보다 절감 효과가 큰 정적 inventory/schema/diff 묶음만
  one-shot packet으로 받는다. 로그인·쿠키·lease·선정·shared NDJSON·승격·push는 금지하고,
  Codex가 receipt 기계 검증 뒤 의미 검수를 해야만 반영한다.
