...
**신규 3기 (09-02 이사님 추가)**: `@heav_gmwin_claude_bot`·`@heav_gmwin_zcode_bot` —
**09-03 합류 시험 합격·정식 편입(명부 갱신)**. `@heav_gmlnx_claude_bot` — **09-04 합류 시험 3항목 통합(ACK·clone 실습 dfe9589·절대금지 요약)·정식 편입(매니저 판정)**.
...
- 접속: `http://43.201.34.144:8023/` + 액세스 토큰(토큰 값은 `.env`에만, 채팅·Git 금지)
- 사용: `@봇 지시` 또는 칩 선택(기본 매니저). 방 로그 최근 24건이 매 봇 구동마다
  주입되어 **봇끼리 서로의 발언이 가시** — 텔레그램에서 불가능하던 부분의 해결.
- 참여: **17기 입장(08-31 11기 + 09-02 firebat 3기 + 09-03 gmwin 2기 + 09-04 gmlnx 1기)** — 매니저·RPG·시나리오·감사·trader·
  자산에이전트·아케이드·소설수집·dev1·dev2·zcode(z.ai 엔진, `zcode.cjs -c -p`)·
  firebat claude/zcode/codex(N100 원격 폴러)·gmlnx claude. **대기 1기**: asus_zai(소재 확인).
- **전체공지 ACK 폐지(09-03)**: @전체 때마다 봇 11기가 전부 기동해 "공지 수신 ✓" 도배
  (이사님: dev2가 자꾸 혼잣말한다 지적) → 공지는 방 로그만 남기고 각 봇 다음 구동 시 문맥 주입으로
  인지. ACK 기동 없음 = 토큰 0.
...
## ★ 매니저 창구 원칙 + 봇 닉네임 표준 (2026-09-06 이사님)

- **창구 원칙**: 매니저(heav_lnx_bot)는 **어디로 이관되든** 이사님과 일하는 단일 창구다.
  구현은 **위치별 봇에 배정**하고, 매니저가 결과를 이사님께 보고한다. AWS→홈 이관 후에도
  창구 기능·채널은 불변 — 이사님 입장에서 대화 창이 바뀌지 않는다.
- **닉네임 표준**(이사님 텔레그램 채팅 목록 관례 채택): 봇 표시명 = **`[위치] 역할`** 형식.
  예: `[AWS 8G] 콘트롤 매니저` · `[AWS 프론트] 콘트롤 aws512` · `[GM 윈도] 콘트롤` ·
  `[감사 CODEX]` · `[AWS 8G] rpg 팀`. 위치가 바뀌면 괄호 안 위치만 갱신한다.
  신규 봇 등록 시 이 형식으로 표시명을 짓는다(roster.json display·회의방 표기 포함).
