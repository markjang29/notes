...
- **봇 대시보드(8025) 설계 완료·검수 대기(09-02 이사님 지시)**: 정본 projects/agent-ops/bot-dashboard-design.md — 위치별 봇 카드(실적 기반 역할 지도)·토큰 트렌드(프록시 로그+usage-log 신설)·사용모델 화면 변경(이사님 토큰 전용 API)·실적 타임라인. 검수=회의방 감사 의뢰 완료 → 통과 후 구현(홈 사이트맵 추가 포함). heav_ai_bot 삭제 반영(총 18봇).
