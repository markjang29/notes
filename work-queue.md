...
- **신규 AWS 엣지 접속 테스트 대기(09-06 이사님)** — 신규 인스턴스 512MB·ubuntu 유저 생성 완료,
  역할=외부 입구 전용(서비스 상주 금지, tailnet → 홈 리눅스 실서빙). 키파일+공인 IP 수령 시
  접속 테스트(512MB·ubuntu 실측) 후 보고. 키는 `~/.ssh` 600 보관, 커밋·채팅 게시 금지.
